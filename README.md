...
<div id="container">
  <div id="gameArea">
    ...
    <div id="progress"></div>
    <div id="controls">
      <button class="btn" onclick="moveBasket('left')">⬅️</button>
      <button class="btn" onclick="moveBasket('right')">➡️</button>
    </div>
  ...
  <div id="scoreboard">
    ...
    <div id="errors">錯誤次數: 0</div>
    <ul id="wordTimes"></ul>
    <button class="btn" onclick="takeScreenshot()">📸 截圖分享</button>
  </div>
</div>
...
<script>
const words = [
  {ch:"卡布奇諾", en:"cappuccino"},
  {ch:"拿鐵", en:"latte"},
  {ch:"美式咖啡", en:"americano"},
  {ch:"濃縮咖啡", en:"espresso"},
  {ch:"瑪奇朵", en:"macchiato"},
  {ch:"咖啡師", en:"barista"},
  {ch:"小杯", en:"tall"},
...
function startWord(){
  ...
  title.textContent="題目："+words[currentWordIndex].ch;
  updateProgress();
  startTime=Date.now();
      ...
      if(lx>left-20 && lx<right){
        if(letter.textContent===word[currentIndex]){
          ...
          if(currentIndex===word.length){
            ...
            li.textContent=words[currentWordIndex].ch+"："+used+" 秒";
            wordTimes.appendChild(li);
            currentWordIndex++;
            if(currentWordIndex<words.length){
              setTimeout(startWord,2000);
            } else {
              timeDisplay.textContent=playerName+" 完成所有單字！總耗時："+totalTime+" 秒";
            }
          }
        } else {
          ...
          totalErrors++;
          errorsDisplay.textContent="錯誤次數: "+totalErrors;
        }
      }
      letter.remove();
